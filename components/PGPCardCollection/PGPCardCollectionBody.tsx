 ...
 */
const PGPCardCollectionBody = ({cards, count, id, $, editKey}: PGPCardCollection): JSX.Element => {
    ...
    return (
        cards && cards?.length > 0 ? <div
            ...
            {...$?.[editKey as keyof LivePreviewTypeMapper<PGPCardCollection>] }
        >
            {cards?.map((cardData: PGPImageCardItem, idx: number) => {
                const prefix = editKey ? `${editKey}__${idx}` : `card__${idx}`
                return (
                    <div
                        {...$?.[prefix as keyof LivePreviewTypeMapper<PGPCardCollection>]}
                        key={idx}
                    >
                        <PGPCard
                            id={id}
